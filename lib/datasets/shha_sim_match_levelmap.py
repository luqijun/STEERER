...
class SHHA_Sim_Match_LevelMap(BaseDataset):
    def __init__(self,
                 config,
                 root,
                 list_path,
        ...
        super(SHHA_Sim_Match_LevelMap, self).__init__(ignore_label, base_size,
        ...
        if 'test' in self.list_path:
            for item in self.img_list:
                ...
                files.append({
                    ...
                    "level_map": f'level_maps/{self.config.dataset.level_nearest_dir}/' + image_id + '.h5',
                    "name": image_id,
                })
        else:
            for item in self.img_list:
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(os.path.join(self.root, item["level_map"]), 'r') as f:
            level_map = np.expand_dims(f['level_map'][:], axis=-1)
            level_map = level_map / 500
        ...
        if self.base_size is not  None:
            image, level_map, points, ratio = self.image_points_resize(image, level_map, self.base_size, points)

        if 'val' in self.list_path or 'test' in self.list_path:
            image, level_map = self.check_img(image, level_map, 32)
            image = self.input_transform(image)
            label = self.label_transform(points, image.shape[:2])
            image = image.transpose((2, 0, 1))
            return image.copy(), label, ratio, name, level_map

        # cv2.imread(os.path.join(self.root, item["label"]),
        #                    cv2.IMREAD_GRAYSCALE)


        image, level_map, label,idx, resize_factor = self.gen_sample(image, level_map, points,name,
                                       self.multi_scale, self.flip,
                                       )

        return image.copy(), label, np.array(size), [name, idx, resize_factor] , level_map


    def image_points_resize(self, image, level_map, long_size, label=None):
        h, w = image.shape[:2]
        ratio = 1.
        if max(h, w)<=long_size:
            if label is not None:
                return image, level_map, label, ratio
            else:
                return image, level_map, label, ratio
        else:
            if h > w:
        ...
        level_map = cv2.resize(level_map, (new_w, new_h),
                           interpolation = cv2.INTER_LINEAR)
        if label is not None:
            label = label * ratio
            return  image, level_map, label, ratio
    ...
    def check_img(self, image, level_map, divisor):
        h, w = image.shape[:2]
        if h % divisor != 0:
        ...
        level_map = self.pad_image(level_map, size=(real_h, real_w), h=h,w=w, padvalue= (255, 255, 255))
        return image, level_map
    ...
    def check_minimum_length(self, image, min_size, label=None):
        ...
        if label is not None:
            ...
            return image,ratio
    def gen_sample(self, image, level_map, points, name, multi_scale=True, is_flip=True):
        ...
        if multi_scale:
            ...
            if not self.AI_resize:
                image, level_map, points = self.crop_then_scale(image, level_map, points, scale_f, self.crop_size)
                idx =[-1]
                resize_factor= [1]
            else:
                return None
        ...
        return image, level_map, label, idx, resize_factor


    def crop_then_scale(self,image, level_map, points, scale_factor, crop_size):
        ...
        level_map = self.pad_image(level_map, h, w, (th, tw), (255, 255, 255))

        h, w = image.shape[:2]
        x = random.randint(0, w - tw)
        y = random.randint(0, h - th)
        image = image[y:y + th, x:x + tw]
        level_map = level_map[y:y + th, x:x + tw]
        ...
        level_map = cv2.resize(level_map, (crop_size[1], crop_size[0]),
                           interpolation=cv2.INTER_LINEAR)  # width, height
        points *= scale_factor

        return image, level_map, points
