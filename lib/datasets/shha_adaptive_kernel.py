...
class SHHA_Adaptive_Kernel(BaseDataset):
    def __init__(self,
                 config,
                 root,
                 list_path,
        ...
        super(SHHA_Adaptive_Kernel, self).__init__(ignore_label, base_size,
        ...
        if 'test' in self.list_path:
            for item in self.img_list:
                ...
                files.append({
                    ...
                    "name": image_id,
                    "gt_den": "gt_adaptive/test/IMG_" + str(int(image_id)-300) + '.h5',
                })
        else:
            for item in self.img_list:
                # import pdb
                ...
                files.append({
                    ...
                    "name": image_id,
                    "weight": 1,
                    "gt_den": "gt_adaptive/train/IMG_" + str(int(image_id)) + '.h5',
                })
        return files
    ...
    def resize_density_map(self, density_map, new_size):
        '''
        重新调整密度图大小，需要归一化
        '''
        # 获取原始密度图的总和（即人数）
        ...
        # 调整密度图的大小
        resized_density_map = cv2.resize(density_map, new_size, interpolation=cv2.INTER_CUBIC)

        # 获取调整大小后的密度图的总和
        resized_sum = np.sum(resized_density_map)

        if resized_sum != 0:
            # 对调整大小后的密度图进行归一化处理，使其总和与原始密度图的总和相等
            normalized_density_map = resized_density_map * (original_sum / resized_sum)
        else:
            normalized_density_map = resized_density_map

        return normalized_density_map

    def image_den_points_resize(self, image, den_map, long_size, label=None):
        h, w = image.shape[:2]
        ratio = 1.
        if max(h, w)<=long_size:
            if label is not None:
                return image, den_map, label,ratio
            else:
                return image, den_map, ratio
        else:
            if h > w:
                new_h = long_size
        ...
        den_map = self.resize_density_map(den_map, (new_w, new_h))
        if label is not None:
            label = label*ratio
        return  image, den_map, label, ratio
    ...
    def __getitem__(self, index):
        ...
        gt_den_map_path = os.path.join(self.root, item["gt_den"])
        with h5py.File(gt_den_map_path, 'r') as f:
            gt_den_map = f['density'][:]

        if gt_den_map.shape != image.shape[:2]:
            h, w = image.shape[:2]
            gt_den_map = self.resize_density_map(gt_den_map, (w, h))

        ratio = 1.0
        if self.base_size is not  None:            
            image, gt_den_map, points, ratio = self.image_den_points_resize(image, gt_den_map, self.base_size, points)

        if self.min_size is not None:
            image, gt_den_map, points, ratio = self.check_minimum_length(image, gt_den_map, self.min_size, points)

        if 'val' in self.list_path or 'test' in self.list_path:
            image, gt_den_map = self.check_img(image, gt_den_map, 32)
            image = self.input_transform(image)
            label = self.label_transform(points, image.shape[:2])
            image = image.transpose((2, 0, 1))

            h, w = gt_den_map.shape[-2:]
            gt_den_map_x1 = np.expand_dims(gt_den_map, axis=0)
            gt_den_map_x2 = np.expand_dims(self.resize_density_map(gt_den_map, (w // 2, h // 2)), axis=0)
            gt_den_map_x4 = np.expand_dims(self.resize_density_map(gt_den_map, (w // 4, h // 4)), axis=0)
            gt_den_map_x8 = np.expand_dims(self.resize_density_map(gt_den_map, (w // 8, h // 8)), axis=0)
            gt_den_map_list = [gt_den_map_x1, gt_den_map_x2, gt_den_map_x4, gt_den_map_x8]

            return image.copy(), label, ratio, name, gt_den_map_list

        # cv2.imread(os.path.join(self.root, item["label"]),
        #                    cv2.IMREAD_GRAYSCALE)

        image, gt_den_map, label,idx,resize_factor = self.gen_sample(image, gt_den_map, points,name,
        ...
        return image.copy(), label, np.array(size), [name, idx, resize_factor], gt_den_map_list

    def check_img(self, image, den_map, divisor):
        h, w = image.shape[:2]
        if h % divisor != 0:
        ...
        den_map = self.pad_image(den_map, size=(real_h, real_w), h=h,w=w, padvalue= (0.0, 0.0, 0.0))
        return image, den_map
    def check_minimum_length(self, image, den_map, min_size, label=None):
        h, w = image.shape[:2]
        ratio = 1.
        if min(h, w)>=min_size:
            if label is not None:
    ...
    def gen_sample(self, image, den_map, points, name, multi_scale=True, is_flip=True):
        ...
        if multi_scale:
            ...
            if not self.AI_resize:
                image, den_map, points = self.crop_then_scale(image, den_map, points, scale_f,self.crop_size)
                idx =[-1]
                resize_factor= [1]
            else:
        ...
        return image, den_map, label, idx, resize_factor
    ...
    def crop_then_scale(self,image, den_map, points, scale_factor, crop_size):
        # th, tw = self.crop_size[0], self.crop_size[1]
        ...
        den_map = self.resize_density_map(den_map, (crop_size[1], crop_size[0]))
        points *= scale_factor

        return image, den_map, points
