...
class SHHA_Sim_Match_LevelMap2(BaseDataset):
    def __init__(self,
                 config,
                 root,
                 list_path,
        ...
        super(SHHA_Sim_Match_LevelMap2, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        if 'val' in self.list_path or 'test' in self.list_path:
            ...
            level_map = np.zeros(image.shape[-2:])
            # img_h, img_w = image.shape[-2:]
            # nearest_points_num = self.config.get('nearest_points_num', 4)
            # level_map = self.compute_image_level(points, img_h, img_w, nearest_points_num)
        ...
        image, label,idx, resize_factor, points = self.gen_sample(image, points,name,
        ...
        img_h, img_w = image.shape[-2:]
        nearest_points_num = self.config.get('nearest_points_num', 4)
        level_map = compute_image_level(points, img_h, img_w, nearest_points_num)
    ...
    def gen_sample(self, image, points, name, multi_scale=True, is_flip=True):
        ...
        return image, label, idx, resize_factor, points
