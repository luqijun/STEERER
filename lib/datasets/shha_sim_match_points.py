...
class SHHA_Sim_Match_Points(BaseDataset):
    def __init__(self,
                 config,
                 root,
                 list_path,
        ...
        super(SHHA_Sim_Match_Points, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        points_format = self.config.get('points_format', "hw")
        ...
        if 'val' in self.list_path or 'test' in self.list_path:
            ...
            targets = {"points": torch.from_numpy(points),
                       "labels": torch.ones([points.shape[0]]).long()}

            return image.copy(), label, targets, ratio, name

        # cv2.imread(os.path.join(self.root, item["label"]),
        #                    cv2.IMREAD_GRAYSCALE)


        image, label, points, idx, resize_factor = self.gen_sample(image, points,name,
                                       self.multi_scale, self.flip,
                                       )
        # construct ch.f.
        # chf = dp.Generating_data_from_dotted_annotation.construct_characteristic_function(points_hw,
        #                                                                                   self.config.network.chf.bandwidth,
        #                                                                                   0,
        #                                                                                   self.config.network.chf.chf_step,
        #                                                                                   self.config.network.chf.chf_tik)
        if points_format == 'hw':
            points = points[:, ::-1].copy()
        targets = { "points": torch.from_numpy(points),
                    "labels": torch.ones([points.shape[0]]).long()}
        return image.copy(), label, targets, np.array(size), [name, idx, resize_factor] #, chf
    ...
    def gen_sample(self, image, points, name, multi_scale=True, is_flip=True):
        ...
        return image, label, points, idx, resize_factor
