...
class DenseScaleNet(nn.Module):
    def __init__(self, fea_channels=512, hidden_channels=256, inner_out_channels=64, load_model=''):
        super(DenseScaleNet, self).__init__()
        self.load_model = load_model
        self.DDCB1 = DDCB(fea_channels, hidden_channels, inner_out_channels)
        self.DDCB2 = DDCB(fea_channels, hidden_channels, inner_out_channels)
        self.DDCB3 = DDCB(fea_channels, hidden_channels, inner_out_channels)
        # self.output_layers = nn.Sequential(nn.Conv2d(fea_channels, 128, 3, padding=1), nn.ReLU(True), nn.Conv2d(128, 64, 3, padding=1), nn.ReLU(True), nn.Conv2d(64, 1, 1))
...
class DDCB(nn.Module):
    def __init__(self, in_planes, hidden_channels=256, inner_out_channels=64):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_planes, hidden_channels, 1, bias=False), nn.ReLU(True),
                                   nn.Conv2d(hidden_channels, inner_out_channels, 3, padding=1, bias=False), nn.ReLU(True))
        self.conv2 = nn.Sequential(nn.Conv2d(in_planes + inner_out_channels, hidden_channels, 1, bias=False), nn.ReLU(True),
                                   nn.Conv2d(hidden_channels, inner_out_channels, 3, padding=2, dilation=2, bias=False), nn.ReLU(True))
        self.conv3 = nn.Sequential(nn.Conv2d(in_planes + inner_out_channels * 2, hidden_channels, 1, bias=False), nn.ReLU(True),
                                   nn.Conv2d(hidden_channels, inner_out_channels, 3, padding=3, dilation=3, bias=False), nn.ReLU(True))
        # 3 × 3 filter size is adopted to fuse concated features from front layers and reduce channel number in the end.
        self.conv4 = nn.Conv2d(in_planes + inner_out_channels * 3, in_planes, 3, bias=False, padding=1)
