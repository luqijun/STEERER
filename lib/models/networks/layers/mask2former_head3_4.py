...
# 使用卷积提取人头特征, 使用卷积+分割query向量 共同生成密度图 取平均值
class Mask2FormerHead3_4(nn.Module):
    ...
    def __init__(self, decoder,
        ...
        super(Mask2FormerHead3_4, self).__init__()
        ...
        self.upsample_block2 = make_head_layer1(feat_channels, feat_channels // 2)
    ...
    def forward(self, x: List[Tensor]):
        ...
        sim_map2 = self.get_sim_map2(mask_features,
                                     self.decoder_positional_encoding(mask),
                                     query_feat,
                                     query_embed,
                                     1)
        sim_map_list.append(sim_map2)

        sim_map_list.append((sim_map + sim_map2) / 2)
    ...
    def get_sim_map2(self, x, x_pos, sim_features, sim_features_pos, layer_num):
        ...
        # get sim map by transformer ouput
        output = output.permute(0, 3, 1, 2)
        sim_map = self.upsample_block2(output)
        return sim_map