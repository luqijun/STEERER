...
# 使用transformer提取人头特征， 分割与生成密度图共用一个查询向量, 生成4个mask
class Mask2FormerHead_v3_1(nn.Module):
    ...
    def __init__(self, decoder,
        ...
        super(Mask2FormerHead_v3_1, self).__init__()
    ...
    def forward(self, x: List[Tensor], **kwargs):
        ...
        point_query_lengths, point_query_feats, point_query_corrds = self.get_point_queries(mask_features, mask_pos, mask_pred)

        img_shape = mask_features.shape[-2:]
        out = self.predict([img_shape[0] * 4 , img_shape[1] * 4], point_query_corrds, point_query_feats, **kwargs)
        out["point_query_lengths"] = point_query_lengths

        return cls_pred_list, mask_pred_list, out


    def get_point_queries(self, mask_features, mask_pos, mask_pred):

        mask_features = mask_features + mask_pos
        seg_mask = (mask_pred.sigmoid() > 0.5).float()
        if seg_mask.shape[-2:] != mask_features.shape[-2:]:
            seg_mask = F.interpolate(seg_mask, size=mask_features.shape[-2:])

        # 使用torch.nonzero()获取值为1的位置坐标
        lengths = []
        point_query_corrds_list = []
        point_query_feats_list = []
        for batch_idx in range(len(seg_mask)):
            indices = torch.nonzero(seg_mask[batch_idx].squeeze(0), as_tuple=True)
            point_query_corrds = torch.Tensor(list(zip(*indices))) * 4 + 2  # [(h_coord, w_coord)...]
            lengths.append(len(point_query_corrds))
            point_query_corrds_list.append(point_query_corrds)

            point_query_feats = mask_features[batch_idx, :, seg_mask[batch_idx].squeeze(0).bool()] # c, b, h, w
            point_query_feats = point_query_feats.permute(1, 0) # (num_queries, dim)
            point_query_feats_list.append(point_query_feats)

        return lengths, point_query_feats_list, point_query_corrds_list
