...
# 使用点查询的的方法：query在原始的64*64特征上采样查询向量，对msdetr特征查询，预测偏置值
class Mask2FormerHead_v3_2(nn.Module):
    ...
    def __init__(self, decoder,
                 ...
                 **kwargs):
        super(Mask2FormerHead_v3_2, self).__init__()
    ...
    def forward(self, x: List[Tensor], **kwargs):
        ...
        points_queries, query_features = self.get_point_queries(origin_features,origin_features.shape[-2:], 4)
        query_result_feas = self.query_crowd_head(query_features, mask_pos, mask_features, mask_pos)

        img_shape = mask_features.shape[-2:]
        out = self.predict([img_shape[0] * 4 , img_shape[1] * 4], points_queries, query_result_feas.flatten(1, 2), **kwargs)
    ...
    def predict(self, img_shape, points_queries, hs, **kwargs):
        ...
        # mask = (outputs_points[:, 0] >= 0) & (outputs_points[:, 0] <= 1) & (outputs_points[:, 1] >= 0) & (outputs_points[:, 1] <= 1)
        # outputs_points = outputs_points[mask]
