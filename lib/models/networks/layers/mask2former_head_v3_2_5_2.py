...
# 使用点查询的的方法：复现P2PNet，使用vgg16_bn
# 使用32*32的特征生成anchor points，使用卷积预测偏置和得分支
# 只有3个scale

class Mask2FormerHead_v3_2_5_2(nn.Module):
    ...
    def __init__(self, decoder,
                 ...
                 **kwargs):
        super(Mask2FormerHead_v3_2_5_2, self).__init__()
        ...
        self.fpn = FPN_Decoder(feat_channels, feat_channels, feat_channels, feat_channels)
    ...
    def forward(self, x: List[Tensor], **kwargs):
        ...
        mask_pred = mask_pred_list[-1] # 32 * 32
        mask_pred = F.interpolate(mask_pred, scale_factor=2) # 128 * 128
        inputs = kwargs['inputs']
        fpn_features = self.fpn(multi_scale_memorys[::-1]) # self.fpn([mask_features, multi_scale_memorys[2], multi_scale_memorys[1]])
        down_features = fpn_features[1]
        # run the regression and classification branch
        regression = self.regression(down_features) * 20  # 8x
        ...
        in_mask_list =[]
        bool_mask_pred = (mask_pred > 0.5)
        temp_output_coord = torch.clamp(output_coord.round(), 0, min(img_h, img_w) - 1)
        temp_output_coord = temp_output_coord[:, :, 0] * temp_output_coord[:, :, 1]
        in_mask1 = bool_mask_pred.flatten(2,3).gather(2, temp_output_coord.unsqueeze(1).long()).squeeze(1)

        y_coords = output_coord[:, :, 0]
        x_coords = output_coord[:, :, 1]
        in_mask2 = (x_coords >= 0) & (x_coords < img_w) & (y_coords >= 0) & (y_coords < img_h)

        in_mask = in_mask1 & in_mask2

        # torch.where(mask, output_coord, torch.zeros_like(points))
        # for coord in output_coord[0]:
        #     c = torch.round(coord).int()
        #     if c[0] >= 0 and c[0] < img_h and c[1] >=0 and c[1] < img_w:
        #         in_mask_list.append(bool_mask_pred[tuple(c)])
        #     else:
        #         in_mask_list.append(torch.Tensor(False))
        #
        # in_mask_list = torch.Tensor(in_mask_list).bool().cuda()
        adjus_scores = classification[in_mask]
        adjus_scores[:, 1] += 0.2
        classification[in_mask] = adjus_scores
