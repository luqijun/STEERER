...
class Mask2FormerHead1(nn.Module):
    ...
    def __init__(self, decoder,
        ...
        super(Mask2FormerHead1, self).__init__()
        ...
        query_sim_feat_num = 16
        self.decoder_positional_encoding = SinePositionalEncoding(**positional_encoding)
        self.query_embed = nn.Embedding(self.num_queries, feat_channels)
        self.query_feat = nn.Embedding(self.num_queries, feat_channels)
        self.query_sim_embed = nn.Embedding(query_sim_feat_num, feat_channels)
        self.query_sim_feat = nn.Embedding(query_sim_feat_num, feat_channels)
        # from low resolution to high resolution
        ...
        # upsample
        self.upsample_block = make_head_layer1(feat_channels, feat_channels // 2)
    ...
    def forward(self, x: List[Tensor]):
        ...
        query_sim_feat = self.query_sim_feat.weight.unsqueeze(0).repeat(
            (batch_size, 1, 1))
        query_sim_embed = self.query_sim_embed.weight.unsqueeze(0).repeat(
            (batch_size, 1, 1))
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            layer_sim = self.transformer_decoder.layers_sim[i]
            sim_attn_mask = attn_mask.repeat(1, query_sim_feat.shape[1], 1)
            query_sim_feat = layer_sim(
                query=query_sim_feat,
                key=decoder_inputs[level_idx],
                value=decoder_inputs[level_idx],
                query_pos=query_sim_embed,
                key_pos=decoder_positional_encodings[level_idx],
                cross_attn_mask=sim_attn_mask,
                query_key_padding_mask=None,
                # here we do not apply masking on padded region
                key_padding_mask=None)

            shape = multi_scale_memorys[level_idx].shape
            sim_map = self.get_sim_map(decoder_inputs[level_idx].permute(0, 2, 1).reshape(shape),
                                       decoder_positional_encodings[level_idx].permute(0, 2, 1).reshape(shape),
                                       query_sim_feat,
                                       query_sim_embed,
                                       i)
    ...
    def get_sim_map(self, x, x_pos, sim_features, sim_features_pos, layer_num):
        ...
        # x = self.dsnet(x)
        B, C, H, W = x.shape
        src = (x + x_pos).permute(0, 2, 3, 1).contiguous()
