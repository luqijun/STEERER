...
# 使用卷积提取人头特征, 使用卷积+分割query向量 共同生成密度图
class Mask2FormerHead3_3_1(nn.Module):
    ...
    def __init__(self, decoder,
        ...
        super(Mask2FormerHead3_3_1, self).__init__()
        ...
        for _ in range(self.num_transformer_decoder_layers + 1):
            transformer = nn.Transformer(feat_channels, 8, num_encoder_layers=2, num_decoder_layers=2, batch_first=True)
            self.sim_map_decoders.append(transformer)
        ...
        # 提取卷积核
        self.conv_kernel_extrator = make_kernel_extractor_adaptive(feat_channels, (7, 7))
    ...
    def get_sim_map(self, x, x_pos, mask_pred, query_fea, query_pos, layer_num):
        ...
        query = query_fea + query_pos
        sim_features = torch.cat([query, kernel], dim=1)
        ...
        for i, window_size in enumerate(self.window_sizes):
            ...
            output = self.sim_map_decoders[layer_num](tgt, output)
