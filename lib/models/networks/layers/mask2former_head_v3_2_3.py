...
# 使用点查询的的方法：不使用查询，参考了P2PNet，使用32*32的特征生成anchor points，使用卷积预测偏置和得分支
# 加入fpn 融合msdetr 特征, 使用第2和第3层和第4层融合
class Mask2FormerHead_v3_2_3_1(nn.Module):
    ...
    def __init__(self, decoder,
                 ...
                 **kwargs):
        super(Mask2FormerHead_v3_2_3_1, self).__init__()
        ...
        self.fpn = FPN_Decoder(256, 256, 256, feat_channels)
    ...
    def forward(self, x: List[Tensor], **kwargs):
        ...
        fpn_features = self.fpn( multi_scale_memorys[::-1])
        down_features = fpn_features[0]
        # run the regression and classification branch
        regression = self.regression(down_features) * 10  # 8x
