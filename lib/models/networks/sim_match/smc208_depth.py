...
class SMC208_Depth(nn.Module):
    ...
    def forward(self, inputs, labels=None, mode='train', *args, **kwargs):
        ...
        if mode == 'val':
            ...
            for i in range(3):
                r_h, r_w = h // 2**(i+1), w // 2**(i+1)
                labels.append(F.interpolate(labels[0], size=(r_h, r_w), mode='nearest'))

        for i, label in enumerate(labels):
            label = label * self.weight
        ...
        gt_masks = [fg_mask_list[2] for _ in range(len(mask_pred_list))]
        ...
        if mode == 'train' or mode == 'val':
            loss = torch.tensor(0., device=labels[0].device)
            if mode == 'train':
                # 计算损失
                loss_fea = self.loss_by_feat(mask_pred_list, gt_masks, sim_map_list, gt_den_maps)
                parsed_losses, log_vars = self.parse_losses(loss_fea)
                loss += parsed_losses
