...
# 使用相似特征库sim_feature_bank，使用rectangle window
class GenerateKernelLayer206(nn.Module):
    def __init__(self, config, kernel_size=3, hidden_channels=256):
        super(GenerateKernelLayer206, self).__init__()
        self.load_model = None
        self.config = config
        self.kernel = None
        self.kernel_size = kernel_size
        ...
        self.window_sizes = [(8, 16), (8, 8), (16, 8),]
    ...
    def forward(self, x, x_with_seg, level):
        ...
        # get sim map by transformer ouput
        sim_map1 = self.upsample_block_2(output)
...
class GenerateKernelLayer207(nn.Module):
    def __init__(self, config, kernel_size=3, hidden_channels=256):
        ...
        num_sim_features = 9
        self.sim_feature_bank = nn.Parameter(torch.randn((num_sim_features, hidden_channels)), requires_grad=True)
        self.transformer = nn.Transformer(hidden_channels, 8, num_encoder_layers=2, num_decoder_layers=2, batch_first=True)
