...
class SMC205_V1(nn.Module):
    def __init__(self, config=None, weight=200, route_size=(64, 64), device=None):
        ...
        # 转换上采样的数据
        self.hidden_channels = config.gen_kernel.hidden_channels
        transition_layers = []
    ...
    def forward(self, inputs, labels=None, mode='train', *args, **kwargs):
        ...
        if self.config.counter_type == 'single_resolution':
            ...
            for i, in_map in enumerate(in_list[::-1]):
                ...
                crop_seg_aggregate = crop_seg_crowd if crop_seg_aggregate is None else (crop_seg_crowd + F.interpolate(crop_seg_aggregate, scale_factor=2, mode='nearest'))
                ...
                # 提取共性特征 获得一个卷积核
                sim_map1, sim_map2 = self.kernel_extractor(last_map, crop_seg_aggregate, len(in_list)- i - 1)
