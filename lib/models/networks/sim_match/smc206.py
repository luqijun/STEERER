...
import math
from .layers import Gaussianlayer, DenseScaleNet, TransitionLayer, SegmentationLayer, GenerateKernelLayer206
from ...losses import CHSLoss2
import logging

# 使用sim_feature_bank存储相似特征  输入特征支持（256, 256） 仅仅在最高层级使用transformer，使用相似性卷积核
...
class SMC206(nn.Module):
    def __init__(self, config=None, weight=200, route_size=(64, 64), device=None):
        ...
        self.seg_crowd = SegmentationLayer(self.hidden_channels, 1, hidden_channels=self.hidden_channels // 2)
        # seg_layers = []
        # for i, c in enumerate(self.config.head.stages_channel):
        #     seg_layers.append(
        ...
        self.kernel_size = 3
        self.kernel_extractor = GenerateKernelLayer206(self.config, self.kernel_size, self.hidden_channels)
    ...
    def forward(self, inputs, labels=None, mode='train', *args, **kwargs):
        ...
        if self.config.counter_type == 'single_resolution':
            ...
            for i, in_map in enumerate(in_list[::-1]):
                ...
                # seg crowd
                if i == 0:
                    seg_crowd_map = self.sgm(self.seg_crowd(in_map))
                else:
                    seg_crowd_map = F.interpolate(fg_list[0], scale_factor=2, mode='nearest')
            ...
            for i, label in enumerate(labels[::-1]):
                ...
                if i==0:
                    fg_mask_list.insert(0, torch.where(label > th, torch.tensor(1), torch.tensor(0)))
                else:
                    fg_mask_list.insert(0, F.interpolate(fg_mask_list[0].float(), scale_factor=2, mode='nearest'))
