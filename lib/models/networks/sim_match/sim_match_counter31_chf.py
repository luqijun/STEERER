...
class SimMatchCounter31_Chf(nn.Module):
    def __init__(self, config=None, weight=200, route_size=(64, 64), device=None):
        super(SimMatchCounter31_Chf, self).__init__()
        self.config = config
        self.device = device
        ...
        self.chf_loss_list = [
            Chfloss(config.chf.chf_step, config.chf.chf_tik, config.chf.sample_interval, config.chf.is_dense)
    ...
    def forward(self, inputs, labels=None, mode='train', *args, **kwargs):
        ...
        if self.config.counter_type == 'single_resolution':
            ...
            # =============================计算Chf损失==============================
            chf = args[0][0] if len(args) > 0 else None
            for i in range(len(out_list)):
                ...
                if sim_map is not None:
                    temp_sim_map = sim_map
                    if temp_sim_map.shape[2:] != sim_map_list[-1].shape[2:]:
                        temp_sim_map = F.interpolate(temp_sim_map, sim_map_list[-1].shape[2:], mode='bilinear')
                    hard_loss2 = 0.0
                    if mode == 'train':
                        hard_loss2 = self.chf_loss_list[i](temp_sim_map / self.weight, chf)
                    ...
                    tmp_loss += hard_loss2
                    out_map_list.append(temp_sim_map)
            ...
            if mode == 'train' or mode == 'val':
                ...
                if mode == 'train':
                    ...
                    result.update({'losses': torch.tensor(0.0) })
