...
from lib.datasets.utils import dataprocessor as dp
from ..losses.chf_loss import Chfloss
...
class Chf_Baseline_Counter(nn.Module):
    def __init__(self, config=None,weight=200, route_size=(64,64),device=None):
        super(Chf_Baseline_Counter, self).__init__()
        self.config = config
        self.device =device
        ...
        self.chf_loss_list = [Chfloss(config.chf.chf_step, config.chf.chf_tik, config.chf.sample_interval, config.chf.is_dense)
                              for _ in range(len(self.resolution_num))]
    ...
    def forward(self,inputs, labels=None, mode='train', *args, **kwargs):
        if self.config.counter_type == 'single_resolution':
            ...
            for i in range(mask_gt.size(1)):
                ...
                # generate chfs
                masked_labels = labels[i].unsqueeze(1) * loss_mask
                chfs = []
                for batch_idx, label in enumerate(masked_labels):
                    coords = torch.nonzero(label.squeeze(0)).float() # 获取非零元素的坐标
                    chf = dp.Generating_data_from_dotted_annotation.construct_characteristic_function(coords,
                                                                                                      self.config.chf.bandwidth,
                                                                                                      0,
                                                                                                      self.config.chf.chf_step,
                                                                                                      self.config.chf.chf_tik)
                    chfs.append(chf.to(self.device))
                chfs = torch.stack(chfs, dim=0)

                hard_loss=self.chf_loss_list[i](out_list[i]*loss_mask / self.weight, chfs)
