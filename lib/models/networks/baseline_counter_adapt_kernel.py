...
class Baseline_Counter_Adapt_Kernel(nn.Module):
    def __init__(self, config=None,weight=200, route_size=(64,64),device=None):
        super(Baseline_Counter_Adapt_Kernel, self).__init__()
        self.config = config
        self.device =device
    ...
    def forward(self,inputs, labels=None, mode='train', *args, **kwargs):
        if self.config.counter_type == 'single_resolution':
            ...
            for i, label in enumerate(args[0][0]):
                label_list.append(label * self.weight)
